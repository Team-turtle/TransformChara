class CharaGraphics {
  ...
  CharaGraphics(String _chara, boolean _reverse) {
    chara = _chara;
    reverse = _reverse;
  ...
  void createModel(String _chara) {
    ...
    PGraphics defChara = createGraphics(800, 300);
    ...
    if (reverse) {
      ...
      defChara.pushMatrix();
      {
        defChara.textAlign(CENTER);
        defChara.scale(1, -1);
        defChara.translate(0, -defChara.height);
        defChara.text(shapingChara, defChara.width/2, defChara.height/2 + 90);
      }
      defChara.popMatrix();
    }
      ...
      for (int i=oldBeginningPixel.x; i<pixels2.length; i++) {
        for (int j=0; j<pixels2[i].length; j++) {
          ...
          if (pixels2[i][j]==color(0)) {
            ...
            if (searchedBorderPixels.get(i+","+j)!=null) continue;
        ...
        //shaping終了
        if (i==pixels2.length-1) break shaping;
      }
      ...
      while (searchOutlinesCount<10000) {
        ...
        for (int i=startDirectionKey; i<startDirectionKey+5; i++) {
          ...
          if (pixels2[searchX][searchY] == color(0)) {
            //探索し始めまで帰ってきたら終了
            if (searchX==beginningPixel.x && searchY==beginningPixel.y) break searchOutlines;
